@extends('templates.content',[
    ...
    'breadcrumbs' => [
        ...
        \App\Classes\Breadcrumb::create('Item Movement Report')
    ]
])
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-3 col-md-12">
                                <div class="input-group float-right">
                                    <a href="/reports/item-movement/export" target="_blank" role="button" class="btn btn-danger btn-flat">
                                        Export
                                    </a>
                                    &nbsp;
                                    &nbsp;
                        ...
                        <div class="col-sm-12">
                            <table id="productTable" class="table table-no-bordered table-hover dataTable dtr-inline" cellspacing="0" width="100%" style="width: 100%;" role="grid" aria-describedby="datatables_info">
                                ...
                                <tr role="row">
                                    ...
                                    <th class="text-right">Pending for Issue</th>
                                ...
                                @if($products->count()>0)
                                    @foreach($products as $product)
                                        ...
                                        <tr role="row" class="{{ $loop->index % 2 ? "odd":"even" }}">
                                            ...
                                            {{--<td>{{ $product->getItemObsolescenceRisk() }}</td>--}}
                                            <td class="text-right">{{ number_format($product->Quantity + $product->getIssuedQuantity() - $product->getReceivedQuantity(),2,'.',',') }}</td>
                                            <td class="text-right">{{ number_format($product->getReceivedQuantity(),2,'.',',') }}</td>
                                            <td class="text-right">{{ number_format($product->getIssuedQuantity(),2,'.',',') }}</td>
                                            <td class="text-right">{{ number_format($product->Quantity,2,'.',',') }}</td>
                                            <td class="text-right">{{ number_format($product->getReservedQuantity(),2,'.',',') }}</td>
                                            <td class="text-right">{{ number_format($product->getIncomingQuantity(),2,'.',',') }}</td>
                                        </tr>
                                    @endforeach
                                @else
                                    <tr>
            ...
            $('.issuance-custom').on('click', function() {
                ...
                swal({
                    ...
                    if(x) {
                        ...
                        window.open('/reports/item-movement/export?start='+dStart+"&end="+dEnd,'_blank');
                    }
                });
