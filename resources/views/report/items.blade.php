@extends('templates.content',[
    ...
    'description'=>'Shows report on recently added items  in the system.',
    'breadcrumbs' => [
        \App\Classes\Breadcrumb::create('Home','/'),
        \App\Classes\Breadcrumb::create('Reports','/reports'),
        \App\Classes\Breadcrumb::create('Recently Added Items')
    ]
])
@section('title','Recent Items Report')
...
@section('content')
    ...
    <div class="card card-danger card-outline flat"> <!--  collapsed-card-->
        <div class="card-header card-header-icon">
            <h4 class="card-title" style="display: inline-block">Recently Added Items</h4>
        </div>
        <div class="card-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-8 col-md-12">
                            <div class="btn-group float-right">
                                <a href="/reports/recent-items/export" target="_blank" class="btn btn-danger btn-flat mr-2 export-today">
                                    Export Today
                                </a>
                    ...
                    @if($products->count()>0)
                        @foreach($products as $product)
                            <tr role="row" class="{{ $loop->index % 2 ? "odd":"even" }}">
                                <td><a class="alert-link" href="/product/view/{{ $product->UniqueID }}">{{ $product->UniqueID }}</a></td>
                                <td>{{ $product->Name }}</td>
                                <td>{{ $product->Description }}</td>
                                <td>{{ \Carbon\Carbon::parse($product->created_at)->format('F d, Y &#8195; h:m:s A') }}</td>
                                {{--<td class="text-right"><a role="button" class="btn btn-sm btn-danger btn-flat" href="/vendor/{{ $product->Code }}/product">View Products</a></td>--}}
                            </tr>
                        @endforeach
                    @else
                        <tr>
            ...
            $('.export-custom').on('click', function() {
                ...
                swal({
                    ...
                    if(x) {
                        ...
                        window.open('/reports/recent-items/export?start='+dStart+"&end="+dEnd,'_blank');
                    }
                });
